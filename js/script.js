...
d3.tsv( "data/CO2-2015.tsv", function ( error, data ) {
  ...
  color.domain( d3.keys( data[ 0 ] ).filter( function ( key ) {
    return key !== "date" && key.slice( -2 ) !== "01" && key.slice( -2 ) !== "02";
  } ) );
      ...
      values: data.map( function ( d ) {
        ...
        for ( var k in d ) {
          if ( k.slice( 0, -2 ) == name && k.slice( -2 ) == 01 ) {
            ...
            var h = +d[ k ];
            comboValues.push( h ); // push values to combo array
          }
        }
        return {
          date: d.date,
  ...
  y.domain( d3.extent( comboValues ) );
  ...
  svg.append( "text" )
    ...
    .attr( "name", function ( d ) {
      return states[ 44 ].name;
    } )
    .attr( "x", width )
    .attr( "y", "0" )
    .text( function ( d ) {
      var type = "co2"
      var state = states[ 44 ].name;
      var amount = states[ 44 ].values[ 13 ][ type ]
      return state + ": " + amount + "%"
    } )
...
function update( tabID ) {
  ...
  var state = svg.selectAll( ".state" ).selectAll( "path" )
